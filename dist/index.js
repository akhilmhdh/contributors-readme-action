...
// node_modules/.pnpm/nanoid@3.3.4/node_modules/nanoid/url-alphabet/index.js
...
// src-ts/query/getSponsorsList.ts
var getSponsorsList_default = `
    query ($owner: String!) {
...
// src-ts/query/getOrgSponsorsList.ts
var getOrgSponsorsList_default = `
    query ($owner: String!) {
...
// src-ts/account.ts
var getKeywordsAccounts = async (octokit, dto) => {
  var _a, _b;
  ...
  const sponsorsList = await octokit.graphql(
    isOrg ? getOrgSponsorsList_default : getSponsorsList_default,
    { owner }
  );
  const contributors = contributorsList.filter((el) => {
    var _a2;
    return el.type !== "Bot" && !((_a2 = el == null ? void 0 : el.login) == null ? void 0 : _a2.includes("actions-user"));
  ...
  const contributorsBots = contributorsList.filter((el) => {
    var _a2;
    return el.type === "Bot" || ((_a2 = el == null ? void 0 : el.login) == null ? void 0 : _a2.includes("actions-user"));
  ...
  const collaborators = collaboratorsList.filter((el) => el.type !== "Bot" && !el.login.includes("actions-user")).map(({ avatar_url: avatarURL = "", login = "", name = "" }) => ({
    avatarURL,
    name,
    type: "person",
    userName: login
  }));
  const collaboratorsBots = contributorsList.filter((el) => {
    var _a2;
    return el.type === "Bot" || ((_a2 = el == null ? void 0 : el.login) == null ? void 0 : _a2.includes("actions-user"));
  }).map(({ login = "", avatar_url: avatarURL = "" }) => ({
    avatarURL,
  ...
  const sponsors = (((_b = (_a = sponsorsList == null ? void 0 : sponsorsList.sponsors) == null ? void 0 : _a.sponsorshipsAsMaintainer) == null ? void 0 : _b.nodes) ?? []).map((el) => {
    if (!el)
      return null;
    if (!el.sponsorEntity)
      return null;
...
var getCustomUsers = async (octokit, keywords) => {
  const userInfo = {};
...
// src-ts/parser/parser.ts
var COMMENT_SIGNATURE_START = "cra:start";
var COMMENT_SIGNATURE_END = "cra:end";
var parser = (fileContent) => {
...
var getKeywords = (line) => {
  const keywords = line.slice(
...
// src-ts/utils/capitalizeWords.ts
var capitalizeWords = (str) => str ? str.split(" ").map((el) => el.charAt(0).toUpperCase() + el.substring(1)).join(" ") : "";

// src-ts/utils/stripDuplicates.ts
var stripDuplicates = (arr, key) => {
  if (!arr || !key)
    return [];
  const hasSeen = {};
  const uniqueArray = [];
  arr.forEach((el) => {
    if (!(hasSeen == null ? void 0 : hasSeen[el[key]])) {
      hasSeen[el[key]] = true;
      uniqueArray.push(el);
    }
  });
...
// src-ts/template/template.ts
var buildTheUserList = (keywords, userList, customUsers) => {
  let users = [];
...
var getTemplate = ({ avatarURL, name, userName }, imageSize = 100) => `
    <td align="center">
...
var templateGenerator = (keywords, userList, customUsers) => {
  ...
  for (let row = 1; row <= rows; row++) {
    ...
    for (let column = 1; column <= columns && (row - 1) * columns + column - 1 < users.length; column++) {
...
async function run() {
  var _a;
  try {
    const path = (0, import_core.getInput)("readme_path").trim();
    const affiliation = (0, import_core.getInput)("collaborators").trim();
    const message = (0, import_core.getInput)("commit_message").trim();
    const name = (0, import_core.getInput)("committer_username").trim();
    const email = (0, import_core.getInput)("committer_email").trim();
    const prTitle = (0, import_core.getInput)("pr_title_on_protected").trim();
    const autoDetectBranchProtection = (0, import_core.getBooleanInput)("auto_detect_branch_protection");
    const { ref } = import_github.context;
    const branch = import_github.context.ref.split("/").pop();
    if (!branch) {
    ...
    const rawContentBase64 = (_a = rawContentRes == null ? void 0 : rawContentRes.data) == null ? void 0 : _a.content;
...
${templateGenerator(c.type, userList, customUserData)}`;
    ...
    const committer = email && name ? { email, name } : void 0;
    if (newContentBase64 !== rawContentBase64) {
      if (isProtected) {
        ...
        (0, import_core.setOutput)("pr_id", prDetails.data.number);
      } else {
        await octokit.rest.repos.createOrUpdateFileContents({
          owner,
          repo,
